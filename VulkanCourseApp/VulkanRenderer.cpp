...
int VulkanRenderer::init(GLFWwindow* newWindow)
{
	...
	try {
		...
		uboViewProjection.projection = glm::perspective(glm::radians(45.0f), (float)swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10000.0f);
		uboViewProjection.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 800.0f), glm::vec3(0.0f, 0.0f, -2.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		Mesh firstMesh = Mesh(mainDevice.physicalDevice, mainDevice.logicalDevice,
			...
			createTexture("Textures/giraffe.jpg"));
		Mesh secondMesh = Mesh(mainDevice.physicalDevice, mainDevice.logicalDevice,
			graphicsQueue, graphicsCommandPool,
			&meshVertices2, &meshIndices,
			createTexture("Textures/panda.jpg"));

		meshList.push_back(firstMesh);
		meshList.push_back(secondMesh);
		*/

		createMeshModel("..\\..\\externals\\Models\\survival-guitar-backpack\\source\\Survival_BackPack_2\\Survival_BackPack_2.fbx");
	}
...
void VulkanRenderer::draw()
{
	...
	{
		throw std::runtime_error("Failed to submit Buffer to Queue");
	}
...
void VulkanRenderer::cleanup()
{
	...
	vkDestroyDescriptorPool(mainDevice.logicalDevice, inputDescriptorPool, NULL);
	vkDestroyDescriptorSetLayout(mainDevice.logicalDevice, inputSetLayout, NULL);

	vkDestroyDescriptorPool(mainDevice.logicalDevice, samplerDescriptorPool, NULL);
	vkDestroyDescriptorSetLayout(mainDevice.logicalDevice, samplerSetLayout, NULL);

	vkDestroySampler(mainDevice.logicalDevice, textureSampler, NULL);

	for (size_t i = 0; i < textureImages.size(); i++)
	{
		vkDestroyImageView(mainDevice.logicalDevice, textureImageViews[i], NULL);
		vkDestroyImage(mainDevice.logicalDevice, textureImages[i], NULL);
		vkFreeMemory(mainDevice.logicalDevice, textureImageMemory[i], NULL);
	}


	for (size_t i = 0; i < depthBufferImages.size(); i++)
	{
		vkDestroyImageView(mainDevice.logicalDevice, depthBufferImageViews[i], NULL);
		vkDestroyImage(mainDevice.logicalDevice, depthBufferImages[i], NULL);
		vkFreeMemory(mainDevice.logicalDevice, depthBufferImageMemorys[i], NULL);
	}

	for (size_t i = 0; i < colorBufferImages.size(); i++)
	{
		vkDestroyImageView(mainDevice.logicalDevice, colorBufferImageViews[i], NULL);
		vkDestroyImage(mainDevice.logicalDevice, colorBufferImages[i], NULL);
		vkFreeMemory(mainDevice.logicalDevice, colorBufferImageMemorys[i], NULL);
	}


	vkDestroyDescriptorPool(mainDevice.logicalDevice, descriptorPool, NULL);
	vkDestroyDescriptorSetLayout(mainDevice.logicalDevice, descriptorSetLayout, NULL);
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		vkDestroyBuffer(mainDevice.logicalDevice, vpUniformBuffer[i], NULL);
		vkFreeMemory(mainDevice.logicalDevice, vpUniformBufferMemory[i], NULL);
		// vkDestroyBuffer(mainDevice.logicalDevice, modelDUniformBuffer[i], NULL);
		// vkFreeMemory(mainDevice.logicalDevice, modelDUniformBufferMemory[i], NULL);
	}
	...
	{
		vkDestroySemaphore(mainDevice.logicalDevice, renderFinished[i], NULL);
		vkDestroySemaphore(mainDevice.logicalDevice, imageAvailable[i], NULL);
		vkDestroyFence(mainDevice.logicalDevice, drawFences[i], NULL);
	}
	vkDestroyCommandPool(mainDevice.logicalDevice, graphicsCommandPool, NULL);
	for (VkFramebuffer framebuffer : swapChainFramebuffers)
	{
		vkDestroyFramebuffer(mainDevice.logicalDevice, framebuffer, NULL);
	}
	vkDestroyPipeline(mainDevice.logicalDevice, secondPipeline, NULL);
	vkDestroyPipelineLayout(mainDevice.logicalDevice, secondPipelineLayout, NULL);
	vkDestroyPipeline(mainDevice.logicalDevice, graphicsPipeline, NULL);
	vkDestroyPipelineLayout(mainDevice.logicalDevice, pipelineLayout, NULL);
	vkDestroyRenderPass(mainDevice.logicalDevice, renderPass, NULL);
...
void VulkanRenderer::createRenderPass()
{
	...
	// ATTACHMENTS
	// SUBPASS 1 ATTACHMENYTS + REFERENCES (INPUT ATTACHMENTS)
	...
	// SUBPASS 2 ATTACHMENYTS + REFERENCES
	...
	// Attachment reference uses an attachment index that refers to index in the attachment list passded to renderPassCreateInfo
	...
	// Reference to attachement that subpass will take input from
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	vpLayoutBinding.pImmutableSamplers = NULL;		// For Texture: Can make sampler data unchangeable (immutable) by specifying in layout
	...
	modelLayoutBinding.pImmutableSamplers = NULL;
	*/
	...
	VkResult result = vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &layoutCreateInfo, NULL, &descriptorSetLayout);
	if (result != VK_SUCCESS)
	{
	...
	// Create Descriptor Set Layout
	result = vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &textureLayoutCreateInfo, NULL, &samplerSetLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Set Layout");
	}
	...
	// Create Descriptor Set Layout
	result = vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &inputLayoutCreateInfo, NULL, &inputSetLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Set Layout");
	}
...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	// How the data for single vertex (including info such as position, color, texture coords, normals, etc) is as a whole
	...
	*********/
	...
	rasterizerCreateInfo.depthBiasEnable = VK_FALSE;		// Whether to add depth bias to fragment (good for stopping "shadow acne" in shadow mapping)
	...
	VkResult result = vkCreatePipelineLayout(mainDevice.logicalDevice, &pipelineLayoutCreateInfo, NULL, &pipelineLayout);
	if (result != VK_SUCCESS)
	{
	...
	depthStencilCreateInfo.depthTestEnable = VK_TRUE;		// Enable checking depth to determine fragmnet write
	...
	pipelineCreateInfo.layout = pipelineLayout;		// Pipeline Layout pipeline should be
	...
	result = vkCreateGraphicsPipelines(mainDevice.logicalDevice, VK_NULL_HANDLE, 1, &pipelineCreateInfo, NULL, &graphicsPipeline);
	if (result != VK_SUCCESS)
	{
	...
	secondPipelineLayoutCreateInfo.pPushConstantRanges = NULL;

	result = vkCreatePipelineLayout(mainDevice.logicalDevice, &secondPipelineLayoutCreateInfo, NULL, &secondPipelineLayout);
	if (result != VK_SUCCESS)
	{
	...
	result = vkCreateGraphicsPipelines(mainDevice.logicalDevice, VK_NULL_HANDLE, 1, &pipelineCreateInfo, NULL, &secondPipeline);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Graphics Pipeline");
	}
	...
	vkDestroyShaderModule(mainDevice.logicalDevice, secondFragmentShaderModule, NULL);
	vkDestroyShaderModule(mainDevice.logicalDevice, secondVertexShaderModule, NULL);
}

void VulkanRenderer::createColorBufferImages()
{
	// Resize supported format for color attachment
	colorBufferImages.resize(swapChainImages.size());
	colorBufferImageMemorys.resize(swapChainImages.size());
	colorBufferImageViews.resize(swapChainImages.size());
	...
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		...
		colorBufferImages[i] = createImage(swapChainExtent.width, swapChainExtent.height, colorFormat, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &colorBufferImageMemorys[i]);
...
void VulkanRenderer::createDepthBufferImages()
{
	...
	depthBufferImageMemorys.resize(swapChainImages.size());
	...
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		...
		depthBufferImages[i] = createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &depthBufferImageMemorys[i]);
...
void VulkanRenderer::createFramebuffers()
{
	...
	for (size_t i = 0; i < swapChainFramebuffers.size(); i++)
	{
		...
		VkResult result = vkCreateFramebuffer(mainDevice.logicalDevice, &framebufferCreateInfo, NULL, &swapChainFramebuffers[i]);
		if (result != VK_SUCCESS)
		{
...
void VulkanRenderer::createCommandPool()
{
	...
	VkResult result = vkCreateCommandPool(mainDevice.logicalDevice, &poolInfo, NULL, &graphicsCommandPool);
	if (result != VK_SUCCESS)
	{
...
{
	MAX_FRAME_DRAWS = swapChainImages.size();
	...
	for (size_t i = 0; i < MAX_FRAME_DRAWS; i++)
	{
		if (vkCreateSemaphore(mainDevice.logicalDevice, &semaphoreCreateInfo, NULL, &imageAvailable[i]) != VK_SUCCESS ||
			vkCreateSemaphore(mainDevice.logicalDevice, &semaphoreCreateInfo, NULL, &renderFinished[i]) != VK_SUCCESS ||
			vkCreateFence(mainDevice.logicalDevice, &fenceCreateInfo, NULL, &drawFences[i]) != VK_SUCCESS)
		{
...
void VulkanRenderer::createTextureSampler()
{
	...
	samplerCreateInfo.anisotropyEnable = VK_TRUE;		// Eanble Anisotropy
	samplerCreateInfo.maxAnisotropy = 16;		// Anisotropy sample level

	VkResult result = vkCreateSampler(mainDevice.logicalDevice, &samplerCreateInfo, NULL, &textureSampler);
	if (result != VK_SUCCESS)
	{
...
void VulkanRenderer::createDescriptorPool()
{
	...
	// Type of descriptor + how many DESCRIPTORS, not Descriptor Sets (combined masks the pool size)
	// ViewProjection Pool
	...
	*/
	...
	VkResult result = vkCreateDescriptorPool(mainDevice.logicalDevice, &poolCreateInfo, NULL, &descriptorPool);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Pool");
	}

	// CREATE SAMPLER SEDCRIPTOR POOL
	...
	result = vkCreateDescriptorPool(mainDevice.logicalDevice, &samplerPoolCreateInfo, NULL, &samplerDescriptorPool);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Pool");
	}
	...
	std::vector<VkDescriptorPoolSize> inputPoolSize = { colorInputPoolSize, depthInputPoolSize };

	// Create input attachment pool
	VkDescriptorPoolCreateInfo inputPoolCreateInfo = {};
	inputPoolCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
	inputPoolCreateInfo.maxSets = swapChainImages.size();
	inputPoolCreateInfo.poolSizeCount = static_cast<uint32_t>(inputPoolSize.size());
	inputPoolCreateInfo.pPoolSizes = inputPoolSize.data();

	result = vkCreateDescriptorPool(mainDevice.logicalDevice, &inputPoolCreateInfo, NULL, &inputDescriptorPool);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Pool");
	}
...
void VulkanRenderer::createDescriptorSets()
{
	...
	setAllocateInfo.pSetLayouts = setLayouts.data();		// Layouts to use to allocate sets (1:1 relationship)
	...
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		...
		vpSetWrite.dstArrayElement = 0;		// Index array to update
		...
		*/
		...
		vkUpdateDescriptorSets(mainDevice.logicalDevice, static_cast<uint32_t>(setWrites.size()), setWrites.data(), 0, NULL);
	}
}
...
void VulkanRenderer::createInputDescriptorSets()
{
	...
	// Fill array of layouts ready for each creation
	...
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		...
		colorAttachmentDescriptor.imageView = colorBufferImageViews[i];
		...
		depthAttachmentDescriptor.imageView = depthBufferImageViews[i];
...
void VulkanRenderer::updateUniformBuffers(uint32_t imageIndex)
{
	...
	{
		UboModel* thisModel =
			reinterpret_cast<UboModel*>((uint64_t)modelTransferSpace + i * modelUniformAlignment);
		...
		vkCmdBeginRenderPass(commandBuffers[currentImage], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
			...
			for (size_t j = 0; j < modelList.size(); j++)
			{
				...
				for (size_t k = 0; k < thisModel.getMeshCount(); k++)
				{
					...
					VkDeviceSize offsets[] = { 0 };		// Offsets into buffers bebing bound
					...
					Model meshModel = {};
					meshModel.model = thisModel.getModel() * thisModel.getMesh(k)->getModel().model;
					...
					vkCmdPushConstants(commandBuffers[currentImage],
						...
						&meshModel);		// Actual data being pushed (can be array)

					// Bind Descriptor Sets
					vkCmdBindDescriptorSets(commandBuffers[currentImage], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
						0, static_cast<uint32_t>(descriptorSetGroup.size()), descriptorSetGroup.data(), 0, NULL);
			...
			vkCmdBindDescriptorSets(commandBuffers[currentImage], VK_PIPELINE_BIND_POINT_GRAPHICS, secondPipelineLayout,
				0, 1, &inputDescriptorSets[currentImage], 0, NULL);
...
void VulkanRenderer::allocateDynamicBufferTransferSpace()
{
	...
	// Create space in memory to hold dynamic buffer that is aligned to our required alignmnet and holds MAX_OBJECTS
...
VkImage VulkanRenderer::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags useFlags, VkMemoryPropertyFlags propFlags, VkDeviceMemory* imageMemory)
{
	...
	imageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;		// Layout of image data in creation
	...
	// Create image
	VkImage image;
	VkResult result = vkCreateImage(mainDevice.logicalDevice, &imageCreateInfo, NULL, &image);
	if (result != VK_SUCCESS)
	{
	...
	result = vkAllocateMemory(mainDevice.logicalDevice, &memoryAllocateInfo, NULL, imageMemory);
	if (result != VK_SUCCESS)
	{
...
VkShaderModule VulkanRenderer::createShaderModule(const std::vector<char>& code)
{
	...
	VkResult result = vkCreateShaderModule(mainDevice.logicalDevice, &shaderModuleCreateInfo, NULL, &shaderModule);
	if (result != VK_SUCCESS)
	{
...
int VulkanRenderer::createTextureImage(std::string fileName)
{
	...
	vkDestroyBuffer(mainDevice.logicalDevice, imageStagingBuffer, NULL);
	vkFreeMemory(mainDevice.logicalDevice, imageStagingBufferMemory, NULL);
...
int VulkanRenderer::createTextureDescriptor(VkImageView textureImage)
{
	...
	vkUpdateDescriptorSets(mainDevice.logicalDevice, 1, &descriptorWrite, 0, NULL);
...
void VulkanRenderer::createMeshModel(std::string modelFile)
{
	...
	std::vector<std::string> textureNames = MeshModel::LoadMaterials(scene, modelFile);
	...
	std::vector<Mesh> modelMeshes = MeshModel::LoadNode(mainDevice.physicalDevice, mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool,
		scene->mRootNode, scene, matToTex, glm::mat4(1.0f));
